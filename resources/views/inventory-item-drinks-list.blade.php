    ...
    <body>
        <div class="container">
            <div class="header-section">
                <h1>Drink Stock List</h1>
                <div>
                    <div class="d-flex justify-content-end position-relative">
                        <a href="{{ route('drinks-item.create') }}" class="btn btn-primary px-2"><i class="fa fa-plus mx-1"
            ...
            <div class="table-container">
                <table class="table table-bordered text-center">
                    ...
                    <tbody>
                        @foreach($drinkItems as $index => $drinkItem)
                            <tr>
                                <td class="text-center"><input type="checkbox" name="selected[]" value="{{ $drinkItem->id }}"
                                        onchange="updateSelectionCount()" />
                                </td>
                                <td class="text-center">{{ $drinkItem->id }}</td>
                                <td class="text-center">{{ $drinkItem->item_name }}</td>
                                <td class="text-center">
                                    {{ $drinkItem->quantity > 0 ? $drinkItem->quantity : 'Out of Stock' }}
                                </td>
                                ...
                                <td class="d-flex justify-content-center">
                                    ...
                                    <form action="{{ route('drinks-item.destroy', $drinkItem->id) }}" method="POST"
                ...
                <nav aria-label="Page navigation example">
                    <ul class="pagination justify-content-center mt-4">
                        <li class="page-item {{ $drinkItems->onFirstPage() ? 'disabled' : '' }}">
                            <a class="page-link" href="{{ $drinkItems->previousPageUrl() }}" tabindex="-1">Previous</a>
                        </li>

                        @foreach(range(1, $drinkItems->lastPage()) as $page)
                            <li class="page-item {{ $page == $drinkItems->currentPage() ? 'active' : '' }}">
                                <a class="page-link" href="{{ $drinkItems->url($page) }}">{{ $page }}</a>
                            </li>
                        @endforeach

                        <li class="page-item {{ !$drinkItems->hasMorePages() ? 'disabled' : '' }}">
                            <a class="page-link" href="{{ $drinkItems->nextPageUrl() }}">Next</a>
                        </li>
                    </ul>
                </nav>
            </div>
